...
describe('chooseStructuredTarget', () => {
  ...
  it('navigates season tabs, pack alternatives, and episodes in visual order',()=>{const seasonOne=item(2,0);const seasonTwo=item(2,1);const packA=item(3,0);const packB=item(3,1);const episode=item(4,0);const elements=[seasonOne,seasonTwo,packA,packB,episode];expect(chooseStructuredTarget(seasonOne,elements,'right')).toBe(seasonTwo);expect(chooseStructuredTarget(seasonTwo,elements,'down')).toBe(packB);expect(chooseStructuredTarget(packB,elements,'left')).toBe(packA);expect(chooseStructuredTarget(packA,elements,'down')).toBe(episode);expect(chooseStructuredTarget(episode,elements,'up')).toBe(packA)});
});
