...
import {API, canonicalHouseholdItems, CatalogDetail, CatalogFacets, CatalogSource, CatalogTitle, Download, DownloadSort, formatBytes, HouseholdItem, HouseholdState, Job, JobLog, LibraryCategory, MediaState, orderDownloadIDs, PlaybackPreferences, reconcileDownloads, Release, resumeActionLabel, resumeForTitle, resumeSummary, seasonPackActionLabel, SubtitleCandidate} from '@filelist/shared';
...
const emptyFacets: CatalogFacets = {categories:[],kinds:[],resolutions:[],hdr:[],sources:[],codecs:[]};
...
function Player({api, download, resumeMs, preferences, onClose, onStateChanged, onComplete}: {api: API; download: Download; resumeMs: number; preferences?:PlaybackPreferences; onClose: () => void; onStateChanged: () => void; onComplete:(preferences:PlaybackPreferences)=>void}) {
  ...
  return <div class="player-shell"><object id="av-player" type="application/avplayer"></object><div class={`player ${controlsVisible ? 'controls-visible' : ''}`}>
    ...
    {menu && <div class="player-dialog">
      ...
      {menu === 'subtitles' && <><h2>Subtitles</h2><button data-focus-region="player-menu" data-focus-row="0" data-focus-col="0" data-focus-key="player-subtitles-off" onClick={() => chooseTrack('TEXT', null)}>Off</button>{subtitleCandidates.map((candidate,index)=><button data-focus-region="player-menu" data-focus-row={index+1} data-focus-col="0" data-focus-key={`player-subtitle-${candidate.provider}-${candidate.id}`} onClick={()=>void installSubtitle(candidate)}><strong>{candidate.fileName||candidate.title||`Included subtitle ${index+1}`}</strong><br/><small>{[candidate.language||'Language unavailable',candidate.providerLabel||candidate.provider,candidate.format].filter(Boolean).join(' · ')}</small></button>)}{subtitleTracks.length>0&&<><h3>Native AVPlay fallback</h3>{subtitleTracks.map((track,index) => <button data-focus-region="player-menu" data-focus-row={subtitleCandidates.length+index+1} data-focus-col="0" data-focus-key={`player-native-subtitle-${track.index}`} onClick={() => chooseTrack('TEXT', track.index)}>{track.label}</button>)}</>}<button data-focus-region="player-menu" data-focus-row={subtitleCandidates.length+subtitleTracks.length+1} data-focus-col="0" data-focus-key="player-subtitles-find" onClick={() => void findSubtitles(false,'remote')}>Find online subtitles…</button></>}
      {menu === 'find-subtitles' && <><h2>Download subtitles</h2>{subtitleCandidates.length === 0 ? <p>No matching provider subtitles are available.</p> : subtitleCandidates.map((candidate,index) => <button data-focus-region="player-menu" data-focus-row={index} data-focus-col="0" data-focus-key={`player-found-subtitle-${candidate.provider}-${candidate.id}`} onClick={() => void installSubtitle(candidate)}><strong>{candidate.fileName||candidate.title||`Subtitle ${index+1}`}</strong><br/><small>{[candidate.language||'Language unavailable',candidate.providerLabel||candidate.provider,candidate.format||'format unavailable',candidate.hearingImpaired?'hearing impaired':''].filter(Boolean).join(' · ')}</small>{candidate.releaseName&&<><br/><small>{candidate.releaseName}</small></>}</button>)}</>}
      {menu === 'options' && <><h2>Playback options</h2><button data-focus-region="player-menu" data-focus-row="0" data-focus-col="0" data-focus-key="player-subtitle-earlier" onClick={() => changeDelay(-500)}>Subtitle earlier (−0.5s)</button><button data-focus-region="player-menu" data-focus-row="1" data-focus-col="0" data-focus-key="player-subtitle-later" onClick={() => changeDelay(500)}>Subtitle later (+0.5s)</button><button data-focus-region="player-menu" data-focus-row="2" data-focus-col="0" data-focus-key="player-subtitle-reset" onClick={() => changeDelay(-subtitleDelay)}>Reset subtitle delay ({subtitleDelay / 1000}s)</button><button data-focus-region="player-menu" data-focus-row="3" data-focus-col="0" data-focus-key="player-aspect-auto" onClick={() => changeAspect('PLAYER_DISPLAY_MODE_AUTO_ASPECT_RATIO')}>Aspect: Auto</button><button data-focus-region="player-menu" data-focus-row="4" data-focus-col="0" data-focus-key="player-aspect-letterbox" onClick={() => changeAspect('PLAYER_DISPLAY_MODE_LETTER_BOX')}>Aspect: Letterbox</button><button data-focus-region="player-menu" data-focus-row="5" data-focus-col="0" data-focus-key="player-aspect-full" onClick={() => changeAspect('PLAYER_DISPLAY_MODE_FULL_SCREEN')}>Aspect: Full screen</button><button data-focus-region="player-menu" data-focus-row="6" data-focus-col="0" data-focus-key="player-info" onClick={() => setMenu('info')}>Playback information</button></>}
      {menu === 'info' && <><h2>Playback information</h2><p>{download.mimeType}</p><p>{formatBytes(download.sizeBytes)} · {tracks.length} tracks</p><p>{formatTime(position)} / {formatTime(total)} · {aspect.replace('PLAYER_DISPLAY_MODE_','')}</p><button data-focus-region="player-menu" data-focus-row="0" data-focus-col="0" data-focus-key="player-info-back" onClick={() => setMenu('options')}>Back to options</button></>}
...
function Setup({draft, server, status, onDraft, onConnect, onForget}: {draft: string; server: string; status: string; onDraft: (value: string) => void; onConnect: () => void; onForget: () => void}) {
  ...
  return <main class="setup"><h1>FileList TV</h1><p>Enter the private-LAN server address.</p><div class="setup-controls"><input ref={address} readOnly data-focus-region="setup" data-focus-row="0" data-focus-col="0" data-focus-key="setup-address" type="url" inputMode="url" autoComplete="off" spellcheck={false} value={draft} onInput={event => onDraft(event.currentTarget.value)} placeholder="http://server.lan:8097"/><button ref={connect} data-focus-region="setup" data-focus-row="0" data-focus-col="1" data-focus-key="setup-connect" class="primary" onClick={onConnect}>Connect</button></div><p class="focus-hint">Select the address and press OK to open the keyboard. Select Connect and press OK when ready.</p><p aria-live="polite">{status}</p>{server && <button data-focus-region="setup" data-focus-row="1" data-focus-col="0" data-focus-key="setup-forget" onClick={onForget}>Forget server</button>}</main>;
}
...
function Catalog({api,status,titles,facets,household,downloads,jobs,restoreFocus,onFocus,onRetry,onChangeServer,onForgetServer,onPlay,onPlayDownload,onManageDownload,onDownloadSeason,onRefreshDownloads,onFavorite}: {api:API; status:string; titles:CatalogTitle[]; facets:CatalogFacets; household:HouseholdState; downloads:Download[]; jobs:Job[]; restoreFocus:string|null; onFocus:(key:string)=>void; onRetry:()=>void; onChangeServer:()=>void; onForgetServer:()=>void; onPlay:(release:Release,fileIndex?:number,resumeMs?:number)=>void; onPlayDownload:(download:Download)=>void; onManageDownload:(download:Download,action:string)=>Promise<void>; onDownloadSeason:(source:CatalogSource,season:number)=>Promise<void>; onRefreshDownloads:()=>Promise<void>; onFavorite:(title:CatalogTitle,value:boolean)=>void}) {
  ...
  const fetchPage=(cursor='',remember=false)=>api.titles({search:query.trim().length>=3?query.trim():undefined,category,sort:effectiveCatalogSort(route,sort),pageSize:12,cursor}).then(page=>{if(remember)setPreviousCursors(current=>[...current,pageCursor].slice(-20));setPageCursor(cursor);setRemoteTitles(page.items);setNextCursor(page.nextCursor);void api.ensureMetadata(page.items.map(item=>item.id));return page;});
  useEffect(()=>{setPreviousCursors([]);setPageNumber(1);void fetchPage('').catch(()=>{})},[query,category,sort,route==='recent']);
  ...
  const downloadSeason=async(source:CatalogSource,season:number)=>{setDetailMessage(`Starting season ${season}…`);try{await onDownloadSeason(source,season);if(detailRef.current){const next=await api.title(detailRef.current.title.id);setDetail(next);setDetailTarget({season})}setDetailMessage(`Season ${season} is downloading. Episode tiles will update here.`)}catch(error){setDetailMessage((error as Error).message)}};
  ...
  const visibleSort=effectiveCatalogSort(route,sort);
  if(visibleSort==='seeders')visible=[...visible].sort((a,b)=>b.bestSeeders-a.bestSeeders);if(visibleSort==='title')visible=[...visible].sort((a,b)=>a.title.localeCompare(b.title));if(visibleSort==='rating')visible=[...visible].sort((a,b)=>Number(Boolean(b.ratingVotes))-Number(Boolean(a.ratingVotes))||(b.rating||0)-(a.rating||0));
  ...
  if(detail)return <TitleDetail key={`${detail.title.id}:${detailTarget.season||0}:${detailTarget.episode||0}`} api={api} detail={detail} target={detailTarget} message={detailMessage} resume={resumeForTitle(household.continueWatching,detail.title.id)} favorite={favoriteIDs.has(detail.title.id)} onClose={onBack} onFavorite={onFavorite} onResume={item=>onPlay(item.release,item.fileIndex,item.positionMs)} onPlay={onPlay} onDownloadSeason={downloadSeason}/>;
  return <div class={`tv-app ${menuOpen?'menu-open':''}`}>
    ...
    <main class="tv-content">
      ...
      {route==='settings'?<TVSettings api={api} onChangeServer={onChangeServer} onForgetServer={onForgetServer}/>:route==='events'?<TVEvents api={api}/>:route==='downloads'?<TVDownloads items={downloads} onPlay={onPlayDownload} onManage={onManageDownload}/>:route==='library-categories'?<TVLibraryCategories api={api} onOpen={openLibraryItem}/>:route==='jobs'?<TVJobs api={api} items={jobs}/>:route==='categories'?<section class="tv-category-grid">{trackerCategories(facets).map((name,index)=><button data-focus-region="content" data-focus-row={3+Math.floor(index/4)} data-focus-col={index%4} data-focus-key={`category-${name}`} onClick={()=>{setCategory(name);setRoute('browse')}}><strong>{name}</strong><span>Browse titles</span></button>)}</section>:<>
        ...
        {['search','browse','recent','tracker'].includes(route)&&rows.every(row=>row.items.length===0)&&<div class="tv-empty"><h2>Nothing here yet</h2><p>Try another section or refresh the catalog.</p></div>}
      </>}
    </main>
  </div>;
}
...
function TVSettings({api,onChangeServer,onForgetServer}:{api:API;onChangeServer:()=>void;onForgetServer:()=>void}){
  ...
  return <section class="tv-settings"><h2>Playback and connection</h2><p>API secrets and filesystem paths stay in browser Settings.</p>{value&&<div class="tv-safe-fields">
    <label>Preferred audio language<input data-focus-region="content" data-focus-row="1" data-focus-col="0" data-focus-key="setting-audio-primary" value={String(value.preferredAudioLanguage||'en')} onInput={event=>setValue({...value,preferredAudioLanguage:event.currentTarget.value})}/></label>
    <label>Preferred subtitle language<input data-focus-region="content" data-focus-row="2" data-focus-col="0" data-focus-key="setting-subtitle-primary" value={String(value.preferredSubtitleLanguage||'')} onInput={event=>setValue({...value,preferredSubtitleLanguage:event.currentTarget.value})}/></label>
    <label>Fallback subtitle language<input data-focus-region="content" data-focus-row="3" data-focus-col="0" data-focus-key="setting-subtitle-fallback" value={String(value.fallbackSubtitleLanguage||'')} onInput={event=>setValue({...value,fallbackSubtitleLanguage:event.currentTarget.value})}/></label>
    <label>Watched threshold<input type="number" data-focus-region="content" data-focus-row="4" data-focus-col="0" data-focus-key="setting-watched" value={String(value.watchedThresholdPercent||90)} onInput={event=>setValue({...value,watchedThresholdPercent:Number(event.currentTarget.value)})}/></label>
    <label>Concurrent background jobs<input type="number" min="1" max="20" data-focus-region="content" data-focus-row="5" data-focus-col="0" data-focus-key="setting-workers" value={String(value.maxConcurrentJobs||10)} onInput={event=>setValue({...value,maxConcurrentJobs:Number(event.currentTarget.value)})}/></label>
    <label>Title refresh timeout (minutes)<input type="number" min="5" max="120" data-focus-region="content" data-focus-row="6" data-focus-col="0" data-focus-key="setting-title-timeout" value={String(value.titleRefreshTimeoutMinutes||30)} onInput={event=>setValue({...value,titleRefreshTimeoutMinutes:Number(event.currentTarget.value)})}/></label>
...
function SourceButton({source,row,onPlay}:{source:CatalogSource;row:number;onPlay:(release:Release,fileIndex?:number)=>void}){return <button class="source-button" data-focus-region="content" data-focus-row={row} data-focus-col="0" data-focus-key={`source-${source.release.id}-${source.fileIndex??-1}`} onClick={()=>onPlay(source.release,source.fileIndex)}><span class="source-copy"><strong>{source.parsed.resolution||'Source'}{source.parsed.hdr?` · ${source.parsed.hdr}`:''}</strong><small class="source-filename">{source.filePath||source.release.name}</small><small>{source.parsed.source||source.release.category} · {source.parsed.videoCodec||'codec unknown'}</small></span><span class="source-action"><TVStateBadges state={source.libraryState}/><b class="source-action-label">{sourceActionLabel(source)}</b><small>{formatBytes(source.fileSizeBytes||source.release.sizeBytes)} · {source.release.seeders} seeders</small></span></button>}

function TVSeasonPackButton({source,season,index,onDownload}:{source:CatalogSource;season:number;index:number;onDownload:(source:CatalogSource,season:number)=>void}){
  const state=source.libraryState;
  const downloaded=state?.downloadState==='downloaded';
  return <button
    disabled={downloaded}
    class="season-pack-button"
    data-focus-region="content"
    data-focus-row="3"
    data-focus-col={index}
    data-focus-key={`season-pack-${source.release.id}`}
    onClick={()=>onDownload(source,season)}
    aria-label={`${source.parsed.resolution||'Season pack'} · ${seasonPackActionLabel(state)}`}
  >
    <span class="season-pack-copy">
      <strong>{source.parsed.resolution||'Season pack'}{source.parsed.hdr?` · ${source.parsed.hdr}`:''}</strong>
      <small class="source-filename">{source.release.name}</small>
      <small>{[source.parsed.source,source.parsed.videoCodec,source.parsed.audio].filter(Boolean).join(' · ')||'Source details unavailable'}</small>
    </span>
    <span class="season-pack-action">
      <TVStateBadges state={state}/>
      <b>{seasonPackActionLabel(state)}</b>
      <small>{formatBytes(source.release.sizeBytes)} · {source.release.seeders} seeders</small>
    </span>
  </button>;
}

function TitleDetail({api,detail,target,message,resume,favorite,onClose,onFavorite,onResume,onPlay,onDownloadSeason}:{api:API;detail:CatalogDetail;target:DetailTarget;message:string;resume?:HouseholdItem;favorite:boolean;onClose:()=>void;onFavorite:(title:CatalogTitle,value:boolean)=>void;onResume:(item:HouseholdItem)=>void;onPlay:(release:Release,fileIndex?:number)=>void;onDownloadSeason:(source:CatalogSource,season:number)=>void}){
  ...
  useEffect(()=>{if(!expanded)return;const key=(event:KeyboardEvent)=>{if(remoteAction(event.key,event.keyCode)!=='back')return;event.preventDefault();event.stopImmediatePropagation();setExpanded('')};document.addEventListener('keydown',key,true);return()=>document.removeEventListener('keydown',key,true)},[expanded]);
  return <main class="detail-screen" style={detail.title.backdropUrl?{backgroundImage:`linear-gradient(90deg,#090d10 5%,rgba(9,13,16,.9) 55%,rgba(9,13,16,.4)),url(${api.streamURL(detail.title.backdropUrl)})`}:undefined}>
    ...
    {detail.seasons.length>0&&<section class="season-browser">
      <h2>Seasons</h2>
      <div class="season-tabs">{detail.seasons.map((item,index)=><button key={item.number} data-focus-region="content" data-focus-row="2" data-focus-col={index} data-focus-key={`season-${item.number}`} class={season===item.number?'active':''} onClick={()=>{setSeason(item.number);setExpanded('')}}><span>Season {item.number}</span><TVStateBadges state={item.libraryState}/></button>)}</div>
      {selected?.packSources&&selected.packSources.length>0&&<div class="season-pack-downloads">
        <h3>Complete season versions</h3>
        <p>Choose one release for every episode. Downloaded versions stay marked; other versions remain available.</p>
        <div class="season-pack-grid">{selected.packSources.map((source,index)=><TVSeasonPackButton key={source.release.id} source={source} season={selected.number} index={index} onDownload={onDownloadSeason}/>)}</div>
      </div>}
...
function App() {
  ...
  async function connect(url = draft) {setStatus('Connecting…'); try {const client = new API(url); const info = await client.info(); const [titlePage,catalogFacets,downloadPage,jobPage]=await Promise.all([client.titles({pageSize:12,sort:'newest'}),client.facets(),client.downloads().catch(()=>({items:[],nextCursor:null,total:0})),client.jobs({pageSize:24}).catch(()=>({items:[],nextCursor:null,total:0}))]);localStorage.setItem(STORAGE,url.replace(/\/$/,''));setServer(url);setAPI(client);setStatus(`${info.name} ${info.version}`);setTitles(titlePage.items);setFacets(catalogFacets);void client.ensureMetadata(titlePage.items.map(item=>item.id)).catch(()=>{});setDownloads(downloadPage.items);setJobs(jobPage.items);await loadState(client);} catch (error) {setStatus((error as Error).message);}}
  ...
  useEffect(()=>{if(!api)return;let stream:EventSource|null=null;let timer=0;let stopped=false;let failures=0;const eventPayload=(event:MessageEvent)=>{const envelope=JSON.parse(event.data);return typeof envelope.payload==='string'?JSON.parse(envelope.payload):envelope.payload};const metadata=(event:MessageEvent)=>{try{const payload=eventPayload(event);const title=payload.title as CatalogTitle|undefined;if(!title?.id)return;setTitles(current=>current.some(item=>item.id===title.id)?current.map(item=>item.id===title.id?title:item):current)}catch(error){void api.diagnostic('warn','Could not process metadata event',{error:String(error)}).catch(()=>{})}};const catalogUpdated=(event:MessageEvent)=>{try{const payload=eventPayload(event);const titleId=String(payload.titleId||payload.title?.id||'');if(titleId)window.dispatchEvent(new CustomEvent('catalog-title-updated',{detail:{titleId}}));setStatus(titleId?'Episode list updated.':'Catalog updated.')}catch(error){void api.diagnostic('warn','Could not process catalog event',{error:String(error)}).catch(()=>{})}};const searchCompleted=(event:MessageEvent)=>{try{const payload=eventPayload(event);window.dispatchEvent(new CustomEvent('catalog-search-completed',{detail:payload}));setStatus(`Search for ${payload.query||'title'} completed.`)}catch(error){void api.diagnostic('warn','Could not process search event',{error:String(error)}).catch(()=>{})}};const open=()=>{if(stopped)return;stream?.close();stream=new EventSource(`${api.base}/api/v1/events`);stream.onopen=()=>{failures=0;setStatus('Server connected')};stream.addEventListener('catalog.updated',catalogUpdated as EventListener);stream.addEventListener('catalog.search.completed',searchCompleted as EventListener);stream.addEventListener('metadata.updated',metadata as EventListener);stream.addEventListener('job.updated',()=>setStatus('A background job was updated.'));stream.onerror=()=>{stream?.close();if(stopped)return;failures++;const delay=Math.min(30_000,1000*Math.pow(2,Math.min(5,failures-1)));setStatus(`Server connection lost. Reconnecting in ${Math.ceil(delay/1000)}s…`);void api.diagnostic('warn','TV event stream disconnected',{attempt:failures}).catch(()=>{});window.clearTimeout(timer);timer=window.setTimeout(open,delay);};};open();return()=>{stopped=true;window.clearTimeout(timer);stream?.close()}},[api]);
