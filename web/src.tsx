...
export function App() {
  ...
  return <div class="app-shell"><Sidebar view={view} onView={guardedNavigate} /><main class="content" id="content"><header class="topbar"><div><h1>{pageTitle}</h1><p>{view === 'home' ? 'Your private screening archive' : view === 'browse' ? 'Every title, grouped and ready to compare' : ''}</p></div><button class="avatar" aria-label="Household profile">H</button></header>{error && <div class="error" role="alert"><strong>Something needs attention</strong><span>{error}</span><button onClick={() => setError('')}>Dismiss</button></div>}
...
function Downloads({ items, onRefresh, onPlay, onRemove, onAction }: { items: Download[]; onRefresh: () => void; onPlay: (d: Download) => void; onRemove: (d: Download) => Promise<void>; onAction: (d: Download, action: DownloadTransferAction) => Promise<void> }) {
...
function Jobs({ onError, deepJobId, onOpenDetail, onCloseDetail }: { onError: (value: string) => void; deepJobId?: string; onOpenDetail: (id: string) => void; onCloseDetail: () => void }) {
  ...
  async function showJob(id: string, announceFailure: boolean) { try { const [result, logs] = await Promise.all([api.job(id), api.jobLogs(id)]); setDetail({ job: result.job, logs: logs.items }); setLogCursor(logs.nextCursor); openJobId.current = id; onOpenDetail(id) } catch (e) { if (announceFailure) onError((e as Error).message) } }
