...
type View = 'home' | 'search' | 'library' | 'continue' | 'favorites' | 'watched' | 'downloads' | 'library-categories' | 'tracker' | 'browse' | 'categories' | 'jobs' | 'events' | 'settings';
...
function App() {
  ...
  async function submitSearch(event?: Event, valueOverride?: string) { event?.preventDefault(); const value = (valueOverride ?? draftQuery).trim(); if (value === query) { void loadTitles(false, ''); return } setSearching(true); setError(''); try { if (value) { const page = await api.searchTitles(value); setTitles(page.items); setNextCursor(page.nextCursor); setHero(page.items[0] || null); void api.ensureMetadata(page.items.slice(0, 12).map(item => item.id)).catch(() => { }); } setQuery(value); setUpdatesAvailable(false) } catch (e) { setError((e as Error).message) } finally { setSearching(false) } }
  ...
  return <div class="app-shell"><Sidebar view={view} onView={v => { setView(v); setDetail(null) }} /><main class="content" id="content"><header class="topbar"><div><h1>{pageTitle}</h1><p>{view === 'home' ? 'Your private screening archive' : view === 'browse' ? 'Every title, grouped and ready to compare' : ''}</p></div><button class="avatar" aria-label="Household profile">H</button></header>{error && <div class="error" role="alert"><strong>Something needs attention</strong><span>{error}</span><button onClick={() => setError('')}>Dismiss</button></div>}
...
function Downloads({ items, onRefresh, onPlay, onRemove }: { items: Download[]; onRefresh: () => void; onPlay: (d: Download) => void; onRemove: (d: Download) => Promise<void> }) {
  ...
  const facts = (download: Download) => [download.parsed?.resolution, download.parsed?.source, download.parsed?.videoCodec, download.parsed?.audio, download.category].filter(Boolean).join(' · ');
...
function Settings({ value, fields, onSaved, onError }: { value: Record<string, unknown>; fields: SettingsField[]; onSaved: (v: Record<string, unknown>) => void; onError: (s: string) => void }) {
  ...
  const groups = [
    ...
    { title: 'qBittorrent and storage', fields: [['qBittorrent URL', 'qbittorrentUrl'], ['qBittorrent username', 'qbittorrentUsername'], ['qBittorrent password', 'qbittorrentPassword', 'password'], ['Download root', 'downloadRoot'], ['Artwork cache path', 'artworkCachePath'], ['Artwork cache maximum bytes', 'artworkCacheMaxBytes', 'number']] },
