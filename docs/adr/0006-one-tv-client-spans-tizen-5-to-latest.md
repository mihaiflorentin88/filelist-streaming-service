...
- **Authoring rules the build cannot check**: flex/grid `gap` (Chromium 84+) is off-limits — spacing is margin-based; `AbortController` (Chromium 66+) must be guarded (LAN discovery depends on it); missing niceties such as native image lazy-loading fall back to eager loading on old engines.
