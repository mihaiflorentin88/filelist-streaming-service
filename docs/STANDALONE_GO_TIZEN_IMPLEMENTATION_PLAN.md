...
Still open before release 1: full compatibility probing/ranking; ratings and uncached extended metadata; complete collection pagination/filtering; dispatcher cancellation/leases/resource budgets; retention/free-space enforcement; and Pi/Tizen end-to-end hardening. Season-pack file expansion, append-only live-search cache growth, library category grouping, and searchable job pagination are implemented. Progressive playback remains an accepted bug until physical playback is demonstrated while qBittorrent progress is below 100%. See [Known issues](KNOWN_ISSUES.md) and the [Tizen verification log](TIZEN.md).
...
The server must never transcode video or audio. On Tizen, “client-side transcoding” must mean direct hardware decoding through Samsung AVPlay and source compatibility selection. Arbitrary real-time transcoding with JavaScript/`ffmpeg.wasm` is not a viable release feature on a TV: it is too slow and memory-heavy and bypasses normal hardware decode paths. Release 1 therefore supports direct play only. The intended release contract is for the server to record stream codecs and rank compatible releases while the TV explains unsupported combinations; compatibility probing/ranking is not implemented at the current checkpoint.
