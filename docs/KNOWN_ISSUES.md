...
Browser video and Tizen AVPlay now retry incomplete streams as requested pieces become readable instead of waiting for 100% completion. The web player decodes non-native audio client-side, and the total-silence defect is fixed and verified on the household Pi: the WASM decoder had been loading the server's 404 page as its binary, and byte-0 decode windows on multi-track files lacked the container header (and re-injected the file's opening audio into every window); the decode route now decodes PCM from both byte 0 and mid-file resumes, and the native route is confirmed healthy. Making every decode failure loud — a visible player error plus automatic fallback to a natively playable track when one exists — is work in flight. Physical S90C AVPlay/remote behavior below 100% still needs observation.
